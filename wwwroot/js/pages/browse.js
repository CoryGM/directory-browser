...
function setupBrowseButtonListener() {
    ...
    browseButton.addEventListener('click', async () => {
        ...
        browseButton.classList.add('busy');

        const currentPath = document.getElementById('current-path-input')?.value ?? '';

        try {
            await pushStateAndGetData(currentPath);
        } finally {
            browseButton.disabled = false;
            browseButton.classList.remove('busy');
        }
    });
}
...
function renderFileTabHtml(data) {
    ...
    data.matchedFileDirectories.forEach((matchedFileDirectory, index) => {
        ...
        if (index === 0) {
            ...
            html += ` ${renderDirectoryLinkHtml(matchedFileDirectory.name, matchedFileDirectory.name)}`;
        }
        ...
        if (matchedFileDirectory.files.length === 0) {
            html +=
                `<tr>
                    <td colspan="3" class="browse-file-tab-file-none-cell">
                        No files
                    </td>
                 </tr>`;
        } else {
...
function renderDirectoryTabHtml(data) {
    ...
    html +=
        `<table id="browse-directory-tab-display-table" class="browse-directory-tab-table">
            <thead>
                <th class="browse-directory-tab-file-name-header">Directory Name</th>
                <th class="browse-directory-tab-file-size-header">Size</th>
                <th class="browse-directory-tab-file-download-header"></th>
            <thead>
        <tbody>`;

    data.matchedDirectories.forEach((matchedDirectory, index) => {
        html +=
            `<tr>
        ...
        if (index === 0) {
            ...
            html += ` ${renderDirectoryLinkHtml(matchedDirectory.name, matchedDirectory.name)}`;
        }
...
function renderDirectoryLinkHtml(url, name) {
    if (!url || !name) return name;

    let html = `<a href="#" class="browse-directory-link" data-dir="${encodeURIComponent(url)}">${name}</a>`;
