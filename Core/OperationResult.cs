...
namespace Browser.Core
{
    ...
    public class OperationResult<T> : OperationResult
    {
        ...
        { 
            get
            {
                // If T is IEnumerable<TItem>, return a clone of the collection
                // to prevent external modification of the internal collection.
                    ...
                    typeof(T).GetGenericTypeDefinition() == typeof(IEnumerable<>))
                {
                    ...
                    // If _data is null, return empty collection
                    if (_data is null)
            ...
            set
            {
                // If the type is a collection and is null, store an empty
                // typed collection instead of null to avoid null reference
                // exceptions in the get.
                if (_data is null &&
                    typeof(T).IsGenericType &&
                    typeof(T).GetGenericTypeDefinition() == typeof(IEnumerable<>))
                {
                    _data = (T)Activator.CreateInstance(typeof(List<>)
